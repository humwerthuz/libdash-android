...
void                LibavDecoder::NotifyVideo             (AVFrame * avFrame, StreamConfig* decoConf)
{
    ...
    for(size_t i = 0; i < videoObservers.size(); i++)
        videoObservers.at(i)->OnVideoDataAvailable((const uint8_t **)&avFrame, &props);
...
{
    /***** ONLY FOR TESTING *****/
    /***** THIS SHOULD BE IN ITS OWN FUNCTION/METHOD *****/

    if(!audioResamplerInited){ /* TODO: WE MUST CHECK IF AUDIO CONTEXT HAS CHANGED */
        av_opt_set_int(audioResampleCtx, "in_channel_layout", decoConf->stream->codec->channel_layout, 0);
        av_opt_set_int(audioResampleCtx, "in_sample_rate", decoConf->stream->codec->sample_rate, 0);
        av_opt_set_int(audioResampleCtx, "in_sample_fmt", decoConf->stream->codec->sample_fmt, 0);
        av_opt_set_int(audioResampleCtx, "out_channel_layout", AV_CH_LAYOUT_STEREO, 0);
        av_opt_set_int(audioResampleCtx, "out_sample_rate", decoConf->stream->codec->sample_rate, 0); /** We must keep the original sample rate, or wild noises appear**/
        av_opt_set_int(audioResampleCtx, "out_sample_fmt", AV_SAMPLE_FMT_S16, 0);

        if(avresample_open(audioResampleCtx) < 0){
            LOGE("FAILED TO OPEN AUDIO RESAMPLING CONTEXT");
            return;
        }
        audioResamplerInited = true;
    }

    int out_linesize;
    uint8_t *output;

    int out_samples = avresample_available(audioResampleCtx) + 
                                          av_rescale_rnd(avresample_get_delay(audioResampleCtx) +
                                                        avFrame->nb_samples, decoConf->stream->codec->sample_rate,
                                                        decoConf->stream->codec->sample_rate,
                                                        AV_ROUND_UP);

    av_samples_alloc(&output, &out_linesize, 2, out_samples, AV_SAMPLE_FMT_S16, 0);

    out_samples = avresample_convert(audioResampleCtx, &output, out_linesize,
                                    out_samples,
    ...
    props.fireError   = false;
    props.streamIndex = decoConf->stream->index;
    props.linesize = out_linesize;
    props.samples = out_samples;
    props.channels = 2; // AV_CH_LAYOUT_STEREO
    props.sampleRate = decoConf->stream->codec->sample_rate;

    for(size_t i = 0; i < audioObservers.size(); i++)
        audioObservers.at(i)->OnAudioDataAvailable((short*)output, &props);
}

AVFormatContext*    LibavDecoder::OpenInput               ()
{
    LOGD("Opening new decoder input stream");

    AVFormatContext *avFormatContextPtr = NULL;

    int             err                 = 0;
    char            * errbuff           = new char[128];
    ...
    if (err < 0)
    {
        ...
        av_strerror(err, errbuff, 128);
        LOGE(("Error while calling avformat_open_input: " + std::string(errbuff)
         + " [CODE " + patch::to_string(err) + "]").c_str());
        return NULL;
    }
    ...
    if (err < 0)
    {
        ...
        av_strerror(err, errbuff, 128);
        LOGE(("Error while calling avformat_find_stream_info: " + std::string(errbuff)).c_str());
         avformat_free_context(avFormatContextPtr);
        return NULL;
    }

    LOGD("Succefully created AVFormatContext");
...
{
    LOGD("Initializing media streams");
...
StreamConfig*       LibavDecoder::GetNextPacket           (AVFormatContext* avFormatContextPtr, AVPacket* avpkt)
{
    while (true)
    {
        ...
        if (err < 0)
        {
            ...
            LOGE("Error while av_read_grame");
            return NULL;
        }
...
int                 LibavDecoder::DecodeFrame             (AVFrame *frame, AVPacket* avpkt, StreamConfig* decConfig)
{
    ...
    while (avpkt->size > 0)
    {
        ...
        if(len < 0)
        {
            ...
            LOGE("Error while decofing frame");
            return -1;
        }
...
bool                LibavDecoder::Init                    ()
{
    ...
    if (this->errorHappened){
    	LOGE("Error happend while opening decoder input");
        return false;
    }
