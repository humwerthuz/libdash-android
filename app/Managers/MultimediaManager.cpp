...
MultimediaManager::MultimediaManager            (DroidGLRenderer *videoElement, DroidAudioRenderer *audioElement) :
...
void*   MultimediaManager::RenderVideo        (void *data)
{
    ...
    LOGD("Video Rendering thread started");

    int64_t processingTimeMS;
    int64_t frameTSMS;
    int refreshCycleMS = (1000 / manager->frameRate);
    int waitTime = 0;
    int waitCounter = 0;

    while(manager->isVideoRendering)
    {
        frameTSMS = getCurrentMillis();
        
        while(!frame) frame = manager->videoStream->GetFrame();

        manager->videoElement->SetImage(frame);
        
        processingTimeMS = getCurrentMillis() - frameTSMS;

        manager->framesDisplayed ++;

        delete frame; //Free our memory... we dont want this bastards lying around,
        frame = NULL; //each one is at least 1MB

        if( refreshCycleMS > processingTimeMS ){
            PortableSleepMS( (refreshCycleMS - processingTimeMS) );
        }

        /*** STATISTICS ***/
        waitCounter ++;
        waitTime += (getCurrentMillis() - frameTSMS);
        if( (waitCounter % 150) == 0 ) LOGD(("AVG Video refresh time: " + patch::to_string(waitTime/waitCounter) + "ms").c_str());
    }
...
void*   MultimediaManager::RenderAudio        (void *data)
{
    ...
    while(manager->isAudioRendering)
    {
        if (samples)
        {
            manager->audioElement->WriteToBuffer(samples);
            // PortableSleep(1 / manager->frameRate);
            // Delay is applied internally.
