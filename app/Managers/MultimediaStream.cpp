...
void        MultimediaStream::Init                      ()
{
    ...
    this->frameBuffer   = new Buffer<RawFrame>(this->frameBufferSize, libdash::framework::buffer::VIDEO);
...
bool        MultimediaStream::StartDownload             ()
{
    if(!dashManager->Start()){
        LOGD("Failed to start MultimediaStream download");
        
        return false; 
    } 

    LOGD("Starting media MultimediaStream download");
    return true;
}
...
void        MultimediaStream::AddFrame                  (RawFrame * frame)
{
    this->frameBuffer->PushBack(frame);
}
RawFrame *  MultimediaStream::GetFrame                  ()
{
